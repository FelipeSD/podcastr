...
export default function Home({latestEpisodes, allEpisodes}: HomeProps) { // props vem do getServerSideProps  
        ...
        <ul>
          {
            latestEpisodes.map(episode => (
              <li key={episode.id}>
                <Image 
                  width={192} height={192} src={episode.thumbnail} alt={episode.title}
                  objectFit="cover"
                />
...
export const getStaticProps: GetStaticProps = async() => { // é chamado pelo next antes de renderizar componente
  ...
  return {
    ...
    revalidate: 60 * 60 * 8, // static site generation a cada 8hrs
  }
}
